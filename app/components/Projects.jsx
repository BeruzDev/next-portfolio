...
const Projects = () => {
	return (
		<div id="projects" className="w-full px-[12%] py-10 scroll-mt-20 mb-24">
      ...
      <h2 className="text-center text-5xl ">Proyectos</h2>
        ...
        Bienvenidos a mi portafolio, donde podrás explorar mis proyectos más recientes y ver cómo he aplicado mis habilidades en el desarrollo web.
			...
			<div className='grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-5 my-10'>
				{projectData.map((project, index) => (
					<div key={index} className='aspect-square bg-no-repeat bg-cover bg-center rounded-lg relative cursor pointer group' style={{ backgroundImage: `url(${project.bgImage})` }}>
						<div className='bg-white w-10/12 rounded-md absolute bottom-5 left-1/2 -translate-x-1/2 py-3 px-5 flex flex-col gap-3 duration-500 group-hover:bottom-7 '> 
							<div>
							...
							<div className='flex flex-row items-center gap-2'>
								<a href={project.githubLink} target="_blank" rel="noopener noreferrer"><Image src={assets.github_icon} alt="GitHub Icon" className='w-6 transition-transform duration-200 hover:scale-110 hover:-translate-y-1' /></a>
								<a href={project.link} target="_blank" rel="noopener noreferrer"><Image src={assets.link_icon} alt="Link Icon" className='w-6 transition-transform duration-200 hover:scale-110 hover:-translate-y-1' /></a>
							</div>
						</div>
					</div>
				))}
